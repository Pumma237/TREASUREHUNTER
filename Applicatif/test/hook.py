...
def deplacement(direction):
	...
	if direction == "haut":
		...
		click_map=[50,400]
	clickGauche(hwnd,click_map)

def havresacZapMalle():
	click_focus=[0,0]
	clickGauche(hwnd,click_focus)
	press('h')
	time.sleep(1)
	click_zaap=[150,350]
	clickGauche(hwnd,click_zaap)
	time.sleep(1)
	click_nomZaap=[550,195]
	clickGauche(hwnd,click_nomZaap)
	press('c','h','a','m','p','s','spacebar','d','e','spacebar','c','a','n','i','a')
	click_seTp=[550,600]
	clickGauche(hwnd,click_seTp)
	time.sleep(1)
	deplacement('droite')
	time.sleep(4)
	deplacement('droite')
...
def destrucs():
  ...
  if (pixel[0]==212) and (pixel[1]==255) and (pixel[2]==0):
  	...
  	print("Chasse PAS en cours !")
  	havresacZapMalle()

  while win32api.GetAsyncKeyState(ord('O')) == 0:
  	time.sleep(1)
